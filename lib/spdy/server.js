...
proto._onConnection = function _onConnection(socket) {
  ...
  if ((!selectedProtocol || !selectedProtocol.match(/spdy|h2/)) &&
      !state.options.debug && !state.options.plain) {
    return state.handler.call(this, socket);
  }

  // TODO(indutny): select protocol, using NPN/ALPN
  var options = util._extend(state.options, {
    protocol: /h2/.test(selectedProtocol) ? spdy.protocol.http2 :
                                            spdy.protocol.spdy
  });

  // Wrap incoming socket into abstract class
  var connection = new Connection(socket, options, this);
  ...
  connection.on('connect', function onconnect(req, socket) {
    ...
    socket.spdyVersion = req.spdyVersion = req.socket._spdyState.version;
  ...
  connection.on('request', function onrequest(req, res) {
    ...
    res.streamID = req.streamID = stream.id;
    res.spdyVersion = req.spdyVersion = stream.version;
