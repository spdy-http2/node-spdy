...
Connection.prototype._setDefaultWindow = function _setDefaultWindow(settings) {
  if (!settings) return;
  if (!settings._initial_window_size) return;

  this.sinkSize = settings._initial_window_size;

  Object.keys(this.streams).forEach(function(id) {
    this.streams[id]._updateWindowSize(settings._initial_window_size);
  }, this);
};
...
Stream.prototype.destroy = function destroy(error) {
  ...
  if (error) {
    if (this.rstCode) {
      this.connection.write(this._framer.rstFrame(this.id, this.rstCode));
    }
  }
...
Stream.prototype._writeData = function _writeData(fin, buffer) {
  ...
  this._lock(function() {
    var stream = this,
        frame = this._framer.dataFrame(this.id, fin, buffer.slice(0, len));
...
//
// ### function _updateWindowSize (size)
// #### @size {Integer}
// Update the internal data transfer window
//
Stream.prototype._updateWindowSize = function _updateWindowSize(size) {
