...
proto._onConnection = function _onConnection(socket) {
  ...
  var connection = new Connection(socket, state);
...
Connection.prototype._init = function init() {
  ...
  this._parser.on('version', function onversion(version) {
    if (!pair) {
      ...
      // Create framer
      self._framer = spdy.protocol.framer.create(version,
                                                 pair.deflate,
                                                 pair.inflate);

      // Send settings frame
      self.write(self._framer.settingsFrame(options));
    }
  });
