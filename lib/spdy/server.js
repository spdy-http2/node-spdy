  ...
  this.parser.on('frame', function (frame) {
    ...
    if (frame.type === 'SYN_STREAM') {
      ...
      // Respond with same PING
        ...
        self.setDefaultWindow(frame.settings);
...
//
// ### function setDefaultWindow (settings)
// #### @settings {Object}
// Update the default transfer window -- in the connection and in the
// active streams
//
Connection.prototype.setDefaultWindow = function(settings) {
  if (!settings) return;
  if (!settings.initial_window_size) return;

  this.sinkSize = settings.initial_window_size;

  Object.keys(this.streams).forEach(function(id) {
    this.streams[id].updateWindowSize(settings.initial_window_size);
  }, this);
};
...
Stream.prototype.drainWindow = function(size) {
  var oldBuffer = this._windowBuffer;
  this._windowBuffer = [];
...
Stream.prototype._writeData = function _writeData(fin, buffer) {
  if (this.framer.version === 3) {
    ...
    if (len < buffer.length) {
      this._windowBuffer.push([fin, buffer.slice(len)]);
      fin = false;
    }
    this._sinkSize -= len;
  }
...
Stream.prototype.end = function end(data, encoding) {
  ...
  if (this._windowBuffer.length !== 0) return;
  this.handleClose();
};
...
Stream.prototype._read = function _read(data) {
  ...
  if (this.framer.version >= 3 && this._initialized) {
    this._windowSize -= data.length;

    if (this._windowSize <= 0) {
      var delta = this._initialSinkSize - this._windowSize;
      this._windowSize += delta;
      this.connection.write(this.framer.windowUpdateFrame(this.id, delta));
    }
  }
...
//
Stream.prototype.updateWindowSize = function updateWindowSize(size) {
...
Stream.prototype.__defineGetter__('remoteAddress', function() {
  return this.socket && this.socket.remoteAddress;
});

Stream.prototype.__defineGetter__('remotePort', function() {
