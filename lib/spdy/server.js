...
Stream.prototype.drainWindow = function(size) {
  var new_backlog = this._backlog;
  this._backlog = [];

  this.internalWindowSize += size;

  for (var i=0; i<new_backlog.length; i++) {
    this._writeData(new_backlog[i][0], new_backlog[i][1]);
  }
}
...
Stream.prototype._writeData = function _writeData(fin, buffer) {
  if (this.internalWindowSize > 0)
    this.internalWindowSize -= buffer.length;

  if (this.internalWindowSize < 0) {
    this._backlog.push([fin, buffer]);
    return;
  }
