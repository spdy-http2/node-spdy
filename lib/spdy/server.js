...
proto.emit = function emit(event, req, res) {
  ...
  if (!(req.socket._handle instanceof spdy.handle)) {
    ...
    req.isSpdy = res.isSpdy = false;
    req.spdyVersion = res.spdyVersion = 1;
    return EventEmitter.prototype.emit.apply(this, arguments);
  }

  var handle = req.connection._handle;

  req.isSpdy = res.isSpdy = true;
  req.spdyVersion = res.spdyVersion = handle.getStream().connection.getVersion();
