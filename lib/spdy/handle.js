...
Handle.prototype._transformHeaders = function _transformHeaders(headers) {
  var res = {};
...
Handle.prototype.emitRequest = function emitRequest() {
  ...
  // TODO(indutny): CONNECT method?
  state.deceiver.emitRequest({
    method: stream.method,
    path: stream.path,
    headers: this._transformHeaders(stream.headers)
  });
};
...
Handle.prototype.emitResponse = function emitResponse(status, headers) {
  ...
  state.deceiver.emitResponse({
    status: status,
    headers: this._transformHeaders(headers)
  });
};
