...
//
// ### function create (inflate)
// #### @inflate {zlib.Inflate} Inflate stream
// @constructor wrapper
//
parser.create = function create(inflate) {
  return new Parser(inflate);
};
...
Parser.prototype.write = function write(data) {
  ...
  this.paused = true;
  this.execute(this.inflate, this.state, buffer, function (err, waiting) {
...
//
// ### function execute (inflate, state, data, callback)
// #### @inflate {zlib.Inflate} Inflate stream
...
// Parse buffered data
//
Parser.prototype.execute = function execute(inflate, state, data, callback) {
  if (state.type === 'frame-head') {
    ...
    spdy.framer.execute(inflate, state.header, data, function(err, frame) {
