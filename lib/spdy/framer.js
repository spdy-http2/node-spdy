...
//
// ### function execute (inflate, header, body, callback)
// #### @inflate {zlib.Inflate} Connection's inflate stream
...
//
framer.execute = function execute(inflate, header, body, callback) {
  // Data frame
  if (!header.control) {
...
//
// ### function sendMaxStreams (connection, count)
// #### @connection {Connection} SPDY Connection
// #### @count {Number} Max Concurrent Streams count
// Sends SETTINGS frame with MAX_CONCURRENT_STREAMS
//
framer.sendMaxStreams = function sendMaxStreams(connection, count) {
  var settings;
