...
Framer.prototype.execute = function execute(header, body, callback) {
  ...
  if (header.type === 0x01 || header.type === 0x02) {
    ...
    var syn_stream = header.type === 0x01,
        id = body.readUInt32BE(0, true) & 0x7fffffff,
        associated = syn_stream ? body.readUInt32BE(4, true) & 0x7fffffff : 0,
  ...
  // RST_STREAM
    ...
    callback(null, {
      type: 'RST_STREAM',
      id: body.readUInt32BE(0, true) & 0x7fffffff,
      status: body.readUInt32BE(4, true)
    });
  // SETTINGS
  ...
  // PING
    ...
    callback(null, {
      type: 'GOAWAY',
      lastId: body.readUInt32BE(0, true) & 0x7fffffff
    });
  } else {
...
Framer.prototype.replyFrame = function replyFrame(id, code, reason, headers,
  ...
  pairsBuf.writeUInt16BE(pairs.length, 0, true);
  pairs.forEach(function (pair) {
    // Write sizes
    pairsBuf.writeUInt16BE(pair[0], offset, true);
    pairsBuf.writeUInt16BE(pair[2], offset + 2 + pair[0], true);
