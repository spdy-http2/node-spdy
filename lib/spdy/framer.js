...
Framer.prototype.execute = function execute(header, body, callback) {
  ...
  if (header.type === 0x01 || header.type === 0x02) {
    ...
    var syn_stream = header.type === 0x01,
        ...
        priority = syn_stream ? body[8] >> 6 : 0,
        headers = {};
...
Framer.prototype.replyFrame = function replyFrame(id, code, reason, headers,
  ...
  var status = code + ' ' + reason,
      version = 'HTTP/1.1',
      size = 2 + // dict size
             2 + 6 + 2 + Buffer.byteLength(status) + // status size
             2 + 7 + 2 + Buffer.byteLength(version); // version size

  pairs = [
    [ 6, 'status', Buffer.byteLength(status), status ],
    [ 7, 'version', Buffer.byteLength(version), version ]
  ].concat(Object.keys(pairs).filter(function (key) {
    var lowerKey = key.toLowerCase();

    if (lowerKey === 'status' || lowerKey === 'version' ||
        lowerKey === 'connection' || lowerKey === 'keep-alive' ||
        lowerKey === 'proxy-connection' || lowerKey === 'transfer-encoding') {
      return false;
    } else {
      return true;
    }
  ...
  spdy.utils.zstream(self.deflate, pairsBuf, function (err, chunks, size) {
    ...
    frame.writeUInt16BE(0x8002, 0); // Control + Version
    frame.writeUInt16BE(0x0002, 2); // SYN_REPLY
    frame.writeUInt32BE((size + 6) & 0x00ffffff, 4); // No flag support

    // Write body start
    frame.writeUInt32BE(id & 0x7fffffff, 8); // Stream-ID
    frame.writeUInt16BE(0, 12); // Unused
...
Framer.prototype.dataFrame = function dataFrame(id, fin, data) {
  ...
  frame.writeUInt32BE(id & 0x7fffffff, 0);
  frame.writeUInt32BE(data.length & 0x00ffffff, 4);
  frame.writeUInt8(fin ? 0x01 : 0x0, 4);
