...
function Stream(connection, options) {
  ...
  if (state.isPush && options.headers && options.headers['content-encoding']) {
    var compression = options.headers['content-encoding'];
...
Stream.prototype._decompress = function decompress(data) {
  ...
  function onError(err) {
    self.emit('error', err);
    state.decompressor.removeListener('data', onData);
  }
  state.decompressor.on('data', onData);
  state.decompressor.once('error', onError);

  // Put data in
  state.decompressor.write(data, function() {
    // Cleanup
    state.decompressor.removeListener('data', onData);
    state.decompressor.removeListener('error', onError);
  });
};
