...
function Stream(connection, options) {
  ...
  state.sinkSize = connection.sinkSize;
  state.initialSinkSize = connection.sinkSize;
  ...
  state.initialWindowSize = connection.windowSize;
  state.windowSize = connection.windowSize;
...
Stream.prototype._isGoaway = function _isGoaway() {
  return this.connection._goaway &&
         this._spdyState.id > this.connection._goaway;
};
...
//
Stream.prototype._init = function init(url, headers) {
  var state = this._spdyState,
      req = [headers.method + ' ' + url + ' ' + headers.version];
...
Stream.prototype._writeData = function _writeData(fin, buffer, cb) {
  ...
  if (state.framer.version === 3) {
    ...
    if (state.sinkSize <= 0) {
      state.sinkBuffer.push([fin, buffer, cb]);
      return false;
    }
  }
  ...
  if (state.framer.version === 3) {
    ...
    if (len < buffer.length) {
      state.sinkBuffer.push([fin, buffer.slice(len), cb]);
