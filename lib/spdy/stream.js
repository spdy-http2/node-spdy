...
Stream.prototype._start = function start(url, headers) {
  ...
  if (this.parser && this.parser.onHeadersComplete)
    this.parser.onHeadersComplete(info);
  else
...
Stream.prototype._recv = function _recv(data) {
  ...
  if (this.parser && this.parser.onBody)
    this.parser.onBody(data, 0, data.length);

  if (spdy.utils.isLegacy) {
    var self = this;
    process.nextTick(function() {
      self.emit('data', data);
      if (self.ondata && (!self.parser || !self.parser.onBody))
        self.ondata(data, 0, data.length);
    });
  } else {
    // Right now, http module expects socket to be working in streams1 mode.
    if (this.ondata && (!this.parser || !this.parser.onBody))
...
// Receive FIN frame
//
Stream.prototype._recvEnd = function _recv() {
  var state = this._spdyState;

  if (this.parser && this.parser.onMessageComplete)
