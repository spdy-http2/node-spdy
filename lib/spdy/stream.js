...
function Stream(connection, options) {
  ...
  state.protocol = connection._spdyState.protocol;
  state.constants = connection._spdyState.constants;
  ...
  state.host = state.headers[':host'] || state.headers[':authority'];
  ...
  state.rstCode = state.constants.error.PROTOCOL_ERROR;
...
Stream.prototype.sendHeaders = function sendHeaders(headers) {
  ...
  state.framer.headersFrame(state.id, headers, function(err) {
    if (err)
...
Stream.prototype.destroy = function destroy(error) {
  ...
  if (error || !state.closedBy.us) {
    if (!state.closedBy.us)
      // CANCEL
      if (state.isClient)
        state.rstCode = state.constants.error.CANCEL;
      // REFUSED_STREAM if terminated before 'finish' event
      else
        state.rstCode = state.constants.error.REFUSED_STREAM;
...
Stream.prototype._writeData = function _writeData(fin, buffer, cb, chunked) {
  ...
  state.framer.dataFrame(frame, function(err) {
    if (err)
      return self.emit('error', err);
...
Stream.prototype._read = function read(bytes) {
  ...
  if (state.framer.version >= 3 &&
    ...
    var self = this;
    state.framer.windowUpdateFrame(state.id, delta, function(err) {
      if (err)
        return self.emit('error', err);
    });
  }
