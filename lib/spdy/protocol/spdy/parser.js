...
Parser.prototype.onSynHeadFrame = function onSynHeadFrame(type,
  ...
  this.parseKVs(body, function(err, headers) {
    ...
    callback(null, {
      ...
      id: head.readUInt32BE() & 0x7fffffff,
      associated: stream ? head.readUInt32BE() & 0x7fffffff : 0,
      priority: stream ? head.readUInt32BE() >> 5 : 0,
      fin: (flags & 0x01) === 0x01,
...
Parser.prototype.parseKVs = function parseKVs(buffer, callback) {
  ...
  // TODO(indutny): update once refactored decompressor/compressor
  this.decompress(buffer.take(buffer.size), function(err, chunks, length) {
    if (err)
      return callback(err);
    ...
    while (count > 0) {
      ...
      if (self.version < 3) {
        var isInternal = /^(method|version|url|scheme)$/.test(key);
        if (key === 'url')
          key = 'path';
        headers[(isInternal ? ':' : '') + key] = value;
      } else {
        headers[key] = value;
      }
      count--;
