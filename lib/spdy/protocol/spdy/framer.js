...
Framer.prototype._frame = function _frame(frame, body, cb) {
  ...
  body(buffer, function(err) {
    ...
    cb(null, Buffer.concat(buffer.render()));
  });
};
...
Framer.prototype.dataFrame = function dataFrame(id, fin, data, callback) {
  ...
  var frame = new WriteBuffer();
  frame.reserve(8 + data.length);

  frame.writeUInt32BE(id & 0x7fffffff);
  frame.writeUInt8(fin ? 0x01 : 0x0);
  frame.writeUInt24BE(data.length);
  frame.copyFrom(data);

  return callback(null, Buffer.concat(frame.render()));
};
