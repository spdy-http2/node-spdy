...
Framer.prototype.headersToDict = function headersToDict(headers,
  ...
  // TODO(indutny): push chunk-by-chunk
  this.compress(Buffer.concat(block.render()), callback);
};
...
Framer.prototype.replyFrame = function replyFrame(frame) {
  ...
  var headers = util._extend({}, frame.headers);
  if (self.version === 2) {
    headers.status = frame.code + ' ' + frame.reason;
  ...
  this.headersToDict(headers, function(err, chunks) {
    if (err)
...
Framer.prototype.streamFrame = function streamFrame(frame) {
  ...
  if (this.version === 2) {
    ...
    if (frame.status)
      headers[':status'] = frame.status;
    headers[':version'] = frame.version || 'HTTP/1.1';
    headers[':path'] = frame.path || frame.url;
  ...
  this.headersToDict(frame.headers, function(err, chunks) {
    if (err)
      return self.emit('error', err);
    ...
    self._frame({
      ...
      buf.writeUInt32BE(frame.id & 0x7fffffff);
      buf.writeUInt32BE(frame.assoc & 0x7fffffff);
...
Framer.prototype.dataFrame = function dataFrame(frame) {
  if (!this.version) {
...
Framer.prototype.goawayFrame = function goawayFrame(frame) {
  this._frame({
    ...
    // Status
    buf.writeUInt32BE(frame.status);
  });
};
