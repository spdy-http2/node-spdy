...
Framer.prototype.execute = function execute(header, body, callback) {
  ...
  if (header.type === 0x01 || header.type === 0x02) {
    var self = this;
    protocol.parseSynHead(header.type,
                          header.flags,
                          body,
                          function(err, frame) {
      ...
      self.inflate(body, function(err, chunks, length) {
        if (err) return callback(err);
        ...
        protocol.parseKVs(pairs, function(err, headers) {
          if (err) return callback(err);
  ...
  // RST_STREAM
    ...
    protocol.parseRst(body, callback);
  // SETTINGS
  } else if (header.type === 0x04) {
    protocol.parseSettings(body, callback);
  ...
  // GOAWAY
    ...
    protocol.parseGoaway(body, callback);
  } else if (header.type === 0x09) {
    protocol.parseWindowUpdate(body, callback);
  } else {
