...
Parser.prototype.execute = function execute(state, data, callback) {
  if (state.type === 'frame-head') {
    ...
    if (this.version === null && header.control) {
      ...
      this.emit('version', header.version);
      this.deflate = spdy.utils.zwrap(this.connection._deflate);
      this.inflate = spdy.utils.zwrap(this.connection._inflate);
    }
...
Parser.prototype.parseSynHead = function parseSynHead(type,
  ...
  this.inflate(kvs, function(err, chunks, length) {
    if (err)
      return callback(err);

    var pairs = Buffer.concat(chunks, length);
    self.parseKVs(pairs, function(err, headers) {
      if (err)
        return callback(err);
