...
Side.prototype.split = function split(data, onChunk, callback) {
  return this._split(data, 0, onChunk, callback);
};

Side.prototype._split = function _split(data, offset, onChunk, callback) {
  if (offset === data.length)
    return process.nextTick(callback);

  var avail = Math.max(0, this.current);
