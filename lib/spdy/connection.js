...
Connection.prototype._sendWindowUpdate = function sendWindowUpdate(force) {
  ...
  state.framer.windowUpdateFrame(0, delta, function(err) {
    if (err)
      return self.emit('error', err);
  });
};
...
Connection.prototype._destroyStreams = function destroyStreams(err) {
  ...
  Object.keys(streams).forEach(function(id) {
    // TODO(indutny): figure out why passing `err` here kills the tests
    streams[id].destroy();
  });
};
...
Connection.prototype._handlePing = function handlePing(opaque, ack) {
  ...
  if (!ack) {
    state.framer.pingFrame(opaque, true, function(err) {
      if (err)
        return self.emit('error', err);
...
Connection.prototype.ping = function ping(callback) {
  ...
  state.framer.pingFrame(opaque, false, function(err) {
    if (err)
      return self.emit('error', err);
