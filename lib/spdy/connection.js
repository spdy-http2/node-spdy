...
Connection.prototype._handleFrame = function handleFrame(frame) {
  ...
  if (frame.type === 'SYN_STREAM') {
    ...
    if (frame.id !== undefined) {
      ...
      if (stream === undefined &&
        ...
        if (frame.type === 'RST_STREAM')
          return;
        this._rst(frame.id, state.constants.rst.INVALID_STREAM);
        return;
      }
    }
    ...
    // Reply for client stream
      ...
      if (!stream._spdyState.isClient) {
        this._rst(frame.id, state.constants.rst.PROTOCOL_ERROR);
        return;
      }
  ...
  if (frame.fin) {
    ...
    if (stream._spdyState.closedBy.them) {
      stream._spdyState.rstCode = state.constants.rst.PROTOCOL_ERROR;
...
Connection.prototype._handleSynStream = function handleSynStream(frame) {
  ...
  if (state.goaway && state.goaway < frame.id)
    return this._rst(frame.id, state.constants.rst.REFUSED_STREAM);

  // PUSH stream
  if (!state.isServer) {
    // Incorrect frame id
    if (frame.id % 2 === 1 || frame.associated % 2 === 0)
      return this._rst(frame.id, state.constants.rst.PROTOCOL_ERROR);
