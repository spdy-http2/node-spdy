...
function Connection(socket, options, server) {
  ...
  state.sentPreface = false;
  state.pair = null;

  // Defaults
  state.maxStreams = options.maxStreams || Infinity;
  state.initialSinkSize = state.constants.DEFAULT_WINDOW;
  ...
  state.windowSize = state.constants.DEFAULT_WINDOW;
  ...
  state.parser = state.protocol.parser.create(this);
...
Connection.prototype._init = function init() {
  ...
  this._spdyState.parser.on('version', this._onVersion.bind(this));

  // Propagate parser errors
  state.parser.on('error', function onParserError(err) {
    // TODO(indutny): handle err.code for ProtocolError
    self.emit('error', err);
  });
...
Connection.prototype._onVersion = function _onVersion(version) {
  ...
  if (!prev) {
    ...
    state.compress = state.pair.compress;
    state.decompress = state.pair.decompress;
    parser.setCompression(state.pair.compress, state.pair.decompress);
    framer.setCompression(state.pair.compress, state.pair.decompress);
  }
  framer.setVersion(version);

  // Send preface+settings frame (once)
  if (!state.sentPreface) {
    state.sentPreface = true;
...
Connection.prototype._handleFrame = function handleFrame(frame) {
  ...
  if (frame.type === 'SYN_STREAM') {
    ...
    if (frame.id !== undefined) {
      ...
      if (stream === undefined &&
        ...
        if (frame.type === 'RST_STREAM')
          return;
        this._rst(frame.id, state.constants.error.INVALID_STREAM);
        return;
      }
    }
    ...
    // Reply for client stream
      ...
      if (!stream._spdyState.isClient) {
        this._rst(frame.id, state.constants.error.PROTOCOL_ERROR);
        return;
      }
    ...
    // Respond with same PING
      ...
      this._handlePing(frame.opaque, frame.ack);
  ...
  if (frame.fin) {
    ...
    if (stream._spdyState.closedBy.them) {
      stream._spdyState.rstCode = state.constants.error.PROTOCOL_ERROR;
...
Connection.prototype._handleSynStream = function handleSynStream(frame) {
  ...
  if (state.goaway && state.goaway < frame.id)
    return this._rst(frame.id, state.constants.error.REFUSED_STREAM);

  // PUSH stream
  if (!state.isServer) {
    // Incorrect frame id
    if (frame.id % 2 === 1 || frame.associated % 2 === 0)
      return this._rst(frame.id, state.constants.error.PROTOCOL_ERROR);
...
Connection.prototype._drainSink = function drainSink(delta) {
  ...
  // Switch to 3.1
  if (state.autoSpdy31 && state.version < 3.1) {
...
Connection.prototype._handlePing = function handlePing(opaque, ack) {
  ...
  if (!ack) {
    state.framer.pingFrame(opaque, true, function(err, frame) {
      if (err)
        return self.emit('error', err);
  ...
  if (!state.pings[hex])
    return;
  var ping = state.pings[hex];
  delete state.pings[hex];
...
Connection.prototype.ping = function ping(callback) {
  ...
  opaque.fill(0);
  opaque.writeUInt32BE(state.pingId, opaque.length - 4);
  state.framer.pingFrame(opaque, false, function(err, frame) {
    if (err)
      return self.emit('error', err);

    state.pings[opaque.toString('hex')] = { cb: callback };
    self.write(frame);
  });
};
