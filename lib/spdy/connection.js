...
Connection.prototype._init = function init() {
  ...
  this._spdyState.parser.on('version', function onversion(version) {
    ...
    if (!prev) {
      pair = pool.get(version);
      state.compress = pair.compress;
      state.decompress = pair.decompress;
      framer.setCompression(pair.compress, pair.decompress);
    }
