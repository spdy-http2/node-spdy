...
function Connection(socket, options) {
  ...
  // Defaults
  state.maxStreams = serverOptions.maxStreams || 100;
  state.sinkSize = serverOptions.sinkSize || (1 << 16);
  state.windowSize = serverOptions.windowSize || (1 << 20);

  // Interleaving configuration
  state.maxChunk = serverOptions.maxChunk || 8 * 1024;
...
Connection.prototype._init = function init() {
  ...
  // Initialize parser
  this._spdyState.parser.on('frame', function (frame) {
    if (state.closed)
      return;
    ...
    if (frame.type === 'SYN_STREAM') {
      stream = new Stream(self, frame);
      self._addStream(stream);

      // If we reached stream limit
      if (state.streamCount > state.maxStreams) {
        stream.once('error', function onerror() {});
        // REFUSED_STREAM
        stream._spdyState.rstCode = 3;
        stream.destroy(true);
      } else {
        self.emit('stream', stream);

        stream._init(frame.url, frame.headers);
      }
    } else {
      ...
      // Respond with same PING
        ...
        self._spdyState.framer.pingFrame(frame.pingId, function(err, frame) {
          if (err)
  ...
  this._spdyState.parser.on('version', function onversion(version) {
    if (!pair) {
      ...
      self._spdyState.version = version;
      self._spdyState.deflate = pair.deflate;
      self._spdyState.inflate = pair.inflate;
