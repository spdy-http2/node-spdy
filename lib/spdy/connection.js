...
function Connection(socket, options) {
  ...
  state.initialSinkSize = 1 << 16;
  state.initialWindowSize = options.windowSize || (1 << 20);
  state.noSpdy31Update = options.noSpdy31Update || !options.plain;

  // Connection-level flow control
  state.sinkSize = state.initialSinkSize;
  state.windowSize = state.initialWindowSize;
...
//
// ### function sendWindowUpdate ()
// Send WINDOW_UPDATE if needed
//
Connection.prototype._sendWindowUpdate = function sendWindowUpdate() {
  var state = this._spdyState;

  if (state.version < 3.1 && (!state.isServer || state.noSpdy31Update) ||
      state.windowSize > 0) {
    return;
  }
...
Connection.prototype._setDefaultWindow = function _setDefaultWindow(settings) {
  ...
  var state = this._spdyState;
  var delta = settings.initial_window_size.value - state.initialSinkSize;
  state.initialSinkSize += delta;
  state.sinkSize += delta;
